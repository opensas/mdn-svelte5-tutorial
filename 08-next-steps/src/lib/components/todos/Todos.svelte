<script lang="ts">
	...
	import type { Filter, Todo as TodoType } from '$lib/components/todos';
	...
	function onremove(todo: TodoType) {
		todos = todos.filter((t) => t.id !== todo.id);
		todosStatus.focus();
		alert.message = `Todo '${todo.name}' has been deleted`;
	}

	function onupdate(todo: TodoType) {
		const i = todos.findIndex((t) => t.id === todo.id);
		if (todos[i].name !== todo.name)
			alert.message = `todo '${todos[i].name}' has been renamed to '${todo.name}'`;
		if (todos[i].completed !== todo.completed)
			alert.message = `todo '${todos[i].name}' marked as ${todo.completed ? 'completed' : 'active'}`;
	...
	function oncheckAll(completed: Todo['completed']) {
